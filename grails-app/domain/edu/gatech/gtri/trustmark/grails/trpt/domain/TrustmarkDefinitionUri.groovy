...
class TrustmarkDefinitionUri implements Uri {
    ...
    static PagedResultList<TrustmarkDefinitionUri> findAllHelper(final long offset, final long max) {
        fromNull((PagedResultList<TrustmarkDefinitionUri>) TrustmarkDefinitionUri.createCriteria().list(offset: offset, max: max) {})
                .orSome(new PagedResultList<TrustmarkDefinitionUri>(null));
    }
}
