...
class TrustmarkStatusReportUri implements Uri {
    ...
    static PagedResultList<TrustmarkStatusReportUri> findAllHelper(final long offset, final long max) {
        fromNull((PagedResultList<TrustmarkStatusReportUri>) TrustmarkStatusReportUri.createCriteria().list(offset: offset, max: max) {})
                .orSome(new PagedResultList<TrustmarkStatusReportUri>(null));
    }
}
