...
class TrustmarkStatusReportUriHistory implements Uri {
    ...
    static mapping = {
        table 'trustmark_status_report_uri_history'
        uri length: 1000
        hash length: 1000
        document type: 'text'
    ...
    TrustmarkStatusReportUriHistory saveAndFlushHelper() {
    ...
    static Option<TrustmarkStatusReportUriHistory> findByIdHelper(final long id) {
        fromNull(findById(id))
    }

    static org.gtri.fj.data.List<TrustmarkStatusReportUriHistory> findAllHelper() {
        fromNull(findAll())
                .map({ collection -> iterableList(collection) })
                .orSome(org.gtri.fj.data.List.<TrustmarkStatusReportUriHistory> nil());
    }
}
